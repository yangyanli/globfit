...
void GlobFit::saveCoaxialOffsets(const std::string& filename)
{
  ...
  for (size_t i = 0, iEnd = _vecPrimitive.size(); i < iEnd; ++ i) {
    ...
    for (size_t j = i + 1, jEnd = iEnd; j < jEnd; ++ j) {
      ...
      fout << -computeCoaxialScore(normal1, point1, point2) << " ";
    }
  }
  fout << std::endl;
...
void GlobFit::saveCoplannarOffsets(const std::string& filename)
{
  std::ofstream fout(filename.c_str());
  if (!fout.good()) {
    return;
  ...
  for (size_t i = 0, iEnd = _vecPrimitive.size(); i < iEnd; ++ i) {
    ...
    for (size_t j = i + 1, jEnd = iEnd; j < jEnd; ++ j) {
      ...
      double d2;
      p2->getDistance(d2);
      fout << -computeCoplanarScore(d1, d2) << " ";
    }
  }
  fout << std::endl;
