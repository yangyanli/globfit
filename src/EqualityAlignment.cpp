...
void GlobFit::saveLengths(const std::string& filename)
{
  ...
  for (size_t i = 0, iEnd = _vecPrimitive.size(); i < iEnd; ++ i) {
    ...
    for (size_t j = 0, jEnd = vecParallelPlane.size(); j < jEnd; ++ j) {
      for (size_t k = j+1, kEnd = vecParallelPlane.size(); k < kEnd; ++ k) {
        ...
        fout << computeLength(normal1, normal2, d1, d2) << " ";
      }
    }
  }
...
void GlobFit::saveRadiuses(const std::string& filename)
{
  std::ofstream fout(filename.c_str());
  if (!fout.good()) {
    return;
  ...
  for (size_t i = 0, iEnd = _vecPrimitive.size(); i < iEnd; ++ i) {
    double radius;
    if (!_vecPrimitive[i]->getRadius(radius)) {
      continue;
    }
    fout << radius << " ";
  }
